...
export function ReservationReportCard({ report }: { report: ReservationReport }) {
  ...
  return (
    <section
      ...
      className="border-2 border-ink bg-paper p-4 shadow-brutal"
    >
      ...
      <p className="mb-3 font-mono text-[11px] uppercase tracking-wider text-ink/50">
        {report.startDate} → {report.endDate}
      </p>

      {held.length > 0 && (
        <div className="mb-3">
          <p className="mb-1.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-ink">
            <FontAwesomeIcon
              icon={faCircleCheck}
          ...
          <ul className="flex flex-wrap gap-1.5">
            {held.map((seat) => (
              <li
                ...
                className="border-2 border-ink bg-seat-team-reserved px-2 py-1 font-mono text-xs font-bold uppercase tracking-wider text-ink"
              >
                {seat.seatLabel}
              </li>
            ))}
      ...
      {unavailable.length > 0 && (
        <div>
          <p className="mb-1.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-ink">
            <FontAwesomeIcon
              icon={faUserLock}
              className="h-3.5 w-3.5 text-bauhaus-red"
              aria-hidden="true"
            />
          ...
          <ul className="flex flex-col gap-1">
            {unavailable.map((seat) => (
              <li
                key={seat.seatId}
                className="flex flex-wrap items-baseline gap-x-2 border-2 border-dashed border-ink/40 px-2.5 py-1.5 text-sm"
              >
