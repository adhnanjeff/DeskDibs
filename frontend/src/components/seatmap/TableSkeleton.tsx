...
export function TableSkeleton({ capacity, label }: TableSkeletonProps) {
  ...
  return (
    <div
      className="relative flex w-full max-w-[220px] min-w-[160px] flex-col gap-1.5 overflow-hidden rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm"
      aria-hidden="true"
    >
      <div className="flex justify-center gap-2">
        {seatsA.map((i) => (
          <div key={`a-${i}`} className="h-3 w-3 rounded-full bg-slate-200" />
        ))}
      </div>
      <div className="h-6 rounded border border-dashed border-slate-200 bg-slate-100" />
      <div className="flex justify-center gap-2">
        {seatsB.map((i) => (
          <div key={`b-${i}`} className="h-3 w-3 rounded-full bg-slate-200" />
        ))}
      </div>
      <div className="mt-0.5 h-2 w-8 self-center rounded bg-slate-200" />
